...
print("Upload data for correlation matrix")
allRes=colnames(subset(UKBB, select= grep("_RES", colnames(UKBB), value = TRUE)))
selRes=paste0(subset(varAll, type=="subjective_fixed")$label, "_RES")
corMatdf=subset(UKBB, select=allRes[allRes %in% selRes])
saveRDS(corMatdf, paste0(HOME,"/output/rds/corMat.rds"))
uploadDropbox(file="corMat.rds", folder="rds")
